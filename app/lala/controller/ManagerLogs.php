 ...
 * 管理员日志管理
 */
class ManagerLogs extends Controller
{
    /**
     * 管理员日志列表
     * @auth true
    ...
    public function index()
    {
        ...
        // 添加基础条件：排除manager_id为0的记录，只查询管理员日志
        $query->where('manager_id', '<>', 0)->where('is_manager', 1);
        
        // 添加搜索条件
        if (isset($get['manager_id']) && $get['manager_id'] !== '') {
            $query->where('manager_id', 'like', "%{$get['manager_id']}%");
        }
        ...
        $result = $query->order('id desc')->paginate([
            ...
            'path' => '/admin.html#/lala/manager_logs/index.html', // 使用正确的路径
        ], false);
    ...
    public function download()
    {
        ...
        // 设置CSV文件名
        $filename = '管理员日志数据_' . date('Y-m-d_H-i-s') . '.csv';
        ...
        foreach ($list as $item) {
            $row = [
                ...
                $this->escapeCsvField("=\"" . ($item['manager_id'] ?? '') . "\""), // 强制文本格式
